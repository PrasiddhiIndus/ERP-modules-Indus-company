...
export const PROFILE_LIST_SELECT =
  "id, email, username, team, role, allowed_modules, allowed_sub_modules, module_access_pending, is_active, created_at";
...
export const PROFILE_AUTH_SELECT =
  "id, email, username, team, role, allowed_modules, allowed_sub_modules, module_access_pending, is_active";
...
export function isMissingProfileModuleAccessPendingError(error) {
  if (!error) return false;
  const msg = [
    error?.message,
    error?.details,
    ...
    .join(" ")
    .toLowerCase();
  return msg.includes("module_access_pending") && (msg.includes("does not exist") || msg.includes("42703"));
}

export function isMissingProfileIsActiveError(error) {
  if (!error) return false;
  const msg = [
    error?.message,
    error?.details,
    ...
    .join(" ")
    .toLowerCase();
  return msg.includes("is_active") && (msg.includes("does not exist") || msg.includes("42703"));
}
