...
const MaintenanceDashboard = () => {
  ...
  useEffect(() => {
    ...
    const revisionSubscription = supabase
      ...
      .on('postgres_changes', 
        { event: 'INSERT', schema: 'public', table: 'maintenance_quotation_revisions' },
        (payload) => {
    ...
    const siteVisitSubscription = supabase
      ...
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'maintenance_site_visits' },
        () => {
          fetchSiteVisitNotifications();
        }
      )
    ...
    try {
      ...
      const query = supabase
        ...
        .select(`
          ...
          status,
          estimated_value,
          maintenance_clients:client_id (id, client_name)
        `)
  ...
  const handleViewAllEnquiries = (e) => {
    ...
    if (dateRange.startDate && dateRange.endDate) {
      navigate(`/app/maintenance/enquiry-master?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`, { replace: false });
    } else {
      navigate('/app/maintenance/enquiry-master', { replace: false });
    }
  };
  ...
  const fetchRevisionNotifications = async () => {
    try {
      ...
      const { data: revisions, error } = await supabase
        ...
        .select(`
          *,
          maintenance_quotations:quotation_id (
            id,
            quotation_number,
            maintenance_clients:client_id (client_name, contact_email)
          )
        `)
  ...
  const dashboardCards = [
    {
      ...
      path: '/app/maintenance/enquiry-master',
      iconWrap: 'bg-red-100 text-red-700',
    },
    {
      icon: RupeeIcon,
      label: 'Quotation Tracker',
      path: '/app/maintenance/quotation-tracker',
      iconWrap: 'bg-green-100 text-green-700',
    },
    {
      icon: Calendar,
      label: 'Follow-up Planner',
      path: '/app/maintenance/follow-up-planner',
      iconWrap: 'bg-violet-100 text-violet-700',
    },
    {
      icon: MapPin,
      label: 'Expo / Visit Tracker',
      path: '/app/maintenance/expo-seminar',
      iconWrap: 'bg-orange-100 text-orange-700',
    },
    {
      icon: ShoppingCart,
      label: 'Contracts / Purchase Orders',
      path: '/app/maintenance/purchase-orders',
      ...
      <div className="mb-6 rounded-2xl border border-slate-200 bg-white/95 shadow-sm p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Maintenance Dashboard</h1>
              <p className="text-sm text-gray-500 mt-0.5">Overview of your maintenance activities</p>
            </div>
          </div>
                        ...
                        {siteVisitNotifications.map((visit) => (
                          <div
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              navigate('/app/maintenance/follow-up-planner', { replace: false });
                              setShowNotifications(false);
                            }}
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <p className="font-medium text-sm text-gray-900">
                                    {revision.maintenance_quotations?.quotation_number || 'Quotation'}
                                  </p>
                                </div>
                                <p className="text-xs text-gray-600 mt-1">
                                ...
                                {revision.maintenance_quotations?.maintenance_clients && (
                                  <p className="text-xs text-gray-500 mt-1">
                                    Client: {revision.maintenance_quotations.maintenance_clients.client_name}
                                  </p>
                                )}
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5">
              ...
              <p className="text-xs text-gray-500 mb-4">Click any action to open the relevant maintenance page.</p>
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[11px] font-medium text-gray-700 truncate mb-0.5">
                                {enquiry.maintenance_clients?.client_name || 'No Client'}
                              </p>
