    ...
    <div className="flex flex-col lg:flex-row gap-0 min-h-[480px] border border-gray-200 rounded-xl overflow-hidden bg-white">
      <aside className="w-full lg:w-[240px] shrink-0 border-b lg:border-b-0 lg:border-r border-gray-200 bg-gray-50/60">
        <div className="p-2 space-y-1">
          ...
          <button type="button" className="w-full mt-2 py-2.5 rounded-lg border border-dashed border-gray-300 text-xs text-gray-500">
            + New package
          </button>
        </div>
      </aside>
      <div className="flex-1 min-w-0 p-4 space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <div className="flex gap-2">
            <button type="button" className="h-8 px-3 rounded-lg bg-[#1F3A8A] text-white text-xs font-medium">Save</button>
            <button type="button" className="h-8 px-3 rounded-lg border text-xs text-red-600">Delete</button>
          </div>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {activePkg.sites.map((site) => (
              <span key={site} className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-800 text-[11px] border border-blue-100">{site}</span>
            ))}
            <button type="button" className="px-2.5 py-1 rounded-full border border-dashed border-gray-300 text-[11px] text-gray-500">+ Add site</button>
          </div>
        </div>
...
export default function FormulaLibrary() {
  ...
  const [packages, setPackages] = useState(() =>
    INITIAL_PACKAGES.map((p) => ({ ...p, formulaIds: [...p.formulaIds], sites: [...p.sites] }))
  );
