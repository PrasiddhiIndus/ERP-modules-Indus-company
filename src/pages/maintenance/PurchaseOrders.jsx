...
const PurchaseOrders = () => {
  ...
  const fetchContracts = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          maintenance_quotations:quotation_id (id, quotation_number),
  ...
  const handleExport = () => {
    const exportData = contracts.map(contract => ({
      ...
      'Quotation Number': contract.maintenance_quotations?.quotation_number || '-',
      'Client': contract.maintenance_clients?.client_name || '-',
                  ...
                  {contracts.map((contract, idx) => (
                    <tr key={contract.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-sm text-gray-500 hidden lg:table-cell">
                        {contract.maintenance_quotations?.quotation_number || '-'}
                      </td>
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-sm text-gray-500">
                        {contract.maintenance_clients?.client_name || '-'}
                      </td>
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-sm text-gray-500">
