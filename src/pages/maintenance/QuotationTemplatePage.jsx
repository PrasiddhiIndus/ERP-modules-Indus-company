  ...
  const loadTemplates = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (
          error.message.includes("maintenance_mail_templates") &&
          error.message.includes("schema cache")
        ) {
          toast.warning("⚠️ Table 'maintenance_mail_templates' not found!\n\nPlease run the SQL migration file:\n'maintenance_mail_templates_schema.sql'\n\nin your Supabase SQL Editor to create the table.");
        } else {
      ...
      {!isFormOpen ? (
        <>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold">Maintenance Mail Templates</h2>
            <button
              onClick={() => {
                resetForm();
                setIsFormOpen(true);
