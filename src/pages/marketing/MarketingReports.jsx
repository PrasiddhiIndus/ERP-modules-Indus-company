...
import {
  ...
  formatMarketingReportError,
  getCustomFieldsForTab,
} from "../../lib/marketingReports";
import { Badge, DenseTable, FilterBar, SectionCard, TinyInput, TinySelect } from "./components/MarketingReportsUi";
...
export default function MarketingReports() {
  ...
  const runReport = useCallback(async () => {
    ...
    try {
      ...
      if (reportType === "custom" || activeTab === "client") {
        data = applyMarketingCustomFilters(data, {
          status: statusFilter,
          clientSearch,
          assigneeSearch,
          search,
      ...
      setRows([]);
      setError(formatMarketingReportError(err));
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const handleExport = () => {
    ...
    exportMarketingReportCsv(rows, cols, fieldDefs, `marketing-${slug}-${fromDate}-to-${toDate}.csv`);
  };
    ...
    <div className="w-full min-h-screen overflow-y-auto bg-gradient-to-b from-slate-50/70 to-white px-4 sm:px-6 py-6">
      <div className="max-w-[1600px] mx-auto space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-white/95 shadow-sm p-4 sm:p-5">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Marketing — Reports & Analytics</h1>
          ...
          {reportType === "custom" ? (
            <div className="mt-3 rounded-lg border border-red-100 bg-red-50/40 p-3 space-y-3">
              ...
              <p className="text-[11px] text-gray-600">
                Choose columns, apply filters above, then run report. Data is pulled live from marketing tables.
              </p>
              <div>
