...
/** PO approval-pending banner removed; executives may still see recently approved POs. */
export default function BillingPoNotificationBar() {
  ...
  if (!approvedForExecutive.length) return null;

  return (
    <div className="mx-4 sm:mx-6 mt-3 rounded-xl border shadow-sm border-emerald-200 bg-emerald-50">
      <div className="px-4 py-3 flex items-center gap-2 border-b border-black/5">
        <CheckCircle className="h-4 w-4 text-emerald-700" />
        <p className="text-sm font-semibold text-gray-900">Recently approved POs</p>
        <span className="text-xs text-gray-600">(latest first)</span>
      </div>
      <ul className="divide-y divide-black/5 max-h-48 overflow-y-auto">
        {approvedForExecutive.map((po) => (
          <li key={po.id} className="px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-sm">
            ...
            <Link to={routeForPo(po)} className="text-xs font-semibold underline text-emerald-800">
              Open PO
            </Link>
          </li>
        ))}
