...
export const ROUTE_MODULE_MAP = [
  ...
  ['/app/admin/employee/official-letters', 'Admin · Official Letters'],
  ['/app/admin/employee/policies', 'Admin · Policies & Terms'],
  ...
  ['/app/admin/alerts-notifications', 'Admin · Alerts & notifications'],
  ['/app/admin/reports-analytics', 'Admin · Reports & analytics'],
  ...
  ['/app/ifsp-employee-master', 'Admin · Employee master'],
  ['/app/ifsp-employee-attendance', 'Admin · Attendance'],
  ['/app/ifsp-employee-payroll', 'Admin · Payroll'],
  ['/app/ifsp-employee-leaves', 'Admin · Leaves'],
  ['/app/store-inventory', 'Admin · Store inventory'],
  ['/app/gate-pass', 'Admin · Gate pass'],
...
const ENTITY_LABELS = {
  ...
  marketing_leads: 'marketing lead',
  ...
  sites: 'site',
  people: 'site worker',
  site_assignments: 'site assignment',
  attendance: 'site attendance mark',
  hr_managers: 'HR manager',
  site_supervisors: 'site supervisor',
...
export const ACTIVITY_DESCRIPTORS = {
  ...
  marketing_leads: {
    INSERT: (req, res) => {
      const row = mergeActivityRows(req, res)
      const name = str(pick(row, 'company'), 48)
      return result(`created marketing lead${name ? ` ${name}` : ''}`, 'CREATED', name || null)
    },
    UPDATE: (req, res) => {
      const row = mergeActivityRows(req, res)
      const name = str(pick(row, 'company'), 48)
      return result(`updated marketing lead${name ? ` ${name}` : ''}`, 'UPDATED', name || null)
    },
    DELETE: (req, res) => {
      const row = mergeActivityRows(req, res)
      const name = str(pick(row, 'company'), 48)
      return result(`deleted marketing lead${name ? ` ${name}` : ''}`, 'DELETED', name || null)
    },
  },
