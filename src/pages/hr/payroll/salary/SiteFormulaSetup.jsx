...
export default function SiteFormulaSetup() {
    ...
    <div className="space-y-4">
      ...
      <SectionCard title="Site" right={<Badge>Formula engine</Badge>}>
        <div className="flex flex-wrap gap-2 items-end mb-4">
          <label className="text-[11px] text-gray-600 flex flex-col gap-1">
            Payroll site
            <TinySelect value={siteId} onChange={(e) => setSiteId(e.target.value)} className="min-w-[200px]">
              {sites.map((s) => (
          ...
          <button type="button" disabled={saving} onClick={save} className="h-8 px-3 rounded-lg bg-[#1F3A8A] text-white text-xs font-medium">
            Save version
          </button>
        ...
        <div className="space-y-6">
          {formulas.map((f) => (
            <div key={f.component_code} className="border border-gray-100 rounded-lg p-3">
              <p className="text-xs font-bold text-gray-800 mb-2">{f.component_code}</p>
              <FormulaBuilder
