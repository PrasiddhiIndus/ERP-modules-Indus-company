...
function statusFilterLabel(value) {
  return TOUR_STATUS_FILTER_OPTIONS.find((o) => o.value === value)?.label || value;
}
...
export function EmployeeTourApprovalsPage() {
  ...
  const refreshCounts = useCallback(async () => {
    try {
      const counts = await fetchTourStatusCounts();
  ...
  const loadRequests = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        setError(e?.message || "Failed to load tour requests.");
      } finally {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeTourWorkflowRealtime(scheduleReload);
    ...
    () =>
      rows.map((r) => {
        ...
        return {
          ...
          dateRange: formatTourDateRange(r.from_date, r.to_date),
          days: r.days,
  ...
  return (
    <div className="space-y-4">
      <PageTaskHeader
        title="Tour request approval"
        subtitle="View tour requests from Indus One. Status shows Pending, Approved, or Rejected, including who approved or rejected each request."
      >
        {realtimeLive ? (
      ...
      >
        <FilterBar>
          <div className="flex flex-col gap-0.5">
            ...
            >
              {TOUR_STATUS_FILTER_OPTIONS.map((o) => (
        ...
        <CollapsibleHelp label="about this list">
          This screen lists tour requests for all employees (not only your own). Approvals and
...
export default EmployeeTourApprovalsPage;
