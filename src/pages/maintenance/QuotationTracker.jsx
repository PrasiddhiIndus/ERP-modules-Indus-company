...
const QuotationTracker = () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          maintenance_clients:client_id (id, client_name, contact_email)
        `)
  ...
  const fetchCostingSheets = async () => {
    try {
      ...
      (data || []).forEach((sheet) => {
        const qNum = sheet?.maintenance_quotations?.quotation_number;
        if (!qNum) return;
  ...
  const fetchQuotationItems = async (quotationId) => {
    try {
      const { data } = await supabase
        .from('maintenance_quotation_items')
        .select('*')
  ...
  const generatePDF = async (quotation) => {
    try {
      ...
      // Client details
      if (quotation.maintenance_clients) {
        doc.setFontSize(12);
        doc.text('To:', 20, yPos);
        doc.setFontSize(10);
        doc.text(quotation.maintenance_clients.client_name, 20, yPos + 5);
        yPos += 15;
      }
  ...
  const handleExport = () => {
    ...
    const exportData = quotations.map(quotation => ({
      ...
      'Client': quotation.maintenance_clients?.client_name || '-',
      'Enquiry Number': quotation.maintenance_enquiries?.enquiry_number || '-',
  ...
  const sendRevisionEmail = async (quotation, remarks) => {
    try {
      ...
      let clientEmail = quotation.maintenance_clients?.contact_email;
      let clientName = quotation.maintenance_clients?.client_name;
                ...
                <table className="w-full min-w-[1000px] text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {quotations.filter(quotation => {
                      ...
                      const enquiryNumber = quotation.maintenance_enquiries?.enquiry_number?.toLowerCase() || '';
                        ...
                        <tr key={quotation.id} className="hover:bg-purple-50/30 transition-colors" data-is-last-row={isLastRow}>
                          ...
                          <td className="px-3 py-2">
                            <span className="text-xs text-gray-600">
                              {quotation.maintenance_enquiries?.enquiry_number || '-'}
                            </span>
                          </td>
                          <td className="px-3 py-2">
                  ...
                  <thead>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {costingSheets.filter((sheet) => {
                        ...
                        const quotation = sheet.maintenance_quotations;
                        const quotationNumber = quotation?.quotation_number?.toLowerCase() || '';
                        const clientName = quotation?.maintenance_clients?.client_name?.toLowerCase() || '';
                        const enquiryNumber = quotation?.maintenance_enquiries?.enquiry_number?.toLowerCase() || '';
                        return quotationNumber.includes(query) || 
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{viewingQuotation.maintenance_clients?.client_name || '-'}</p>
                  </div>
                  <div>
