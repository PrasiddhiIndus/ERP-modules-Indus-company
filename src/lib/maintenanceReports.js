/**
 * Maintenance module reports — enquiries, quotations, clients, follow-ups, contracts.
 */
...
export function normalizeEnquiryRow(row) {
  return {
    ...
    source: row.source || "",
    clientName: row.maintenance_clients?.client_name || "",
...
export function normalizeQuotationRow(row) {
  return {
    ...
    clientName: row.maintenance_clients?.client_name || "",
    enquiryNumber: row.maintenance_enquiries?.enquiry_number || "",
...
export function normalizeFollowUpRow(row) {
  const client =
    row.maintenance_enquiries?.maintenance_clients?.client_name ||
    row.maintenance_quotations?.maintenance_clients?.client_name ||
    "";
  return {
    id: row.id,
    ...
    status: row.status || "",
    enquiryNumber: row.maintenance_enquiries?.enquiry_number || "",
    quotationNumber: row.maintenance_quotations?.quotation_number || "",
    clientName: client,
    remarks: row.remarks || "",
    createdAt: row.created_at || "",
  };
...
export async function fetchEnquiryReportRows(supabase, { fromDate, toDate } = {}) {
  let query = supabase
    ...
    .select(`*, maintenance_clients:client_id (client_name)`)
...
export async function fetchQuotationReportRows(supabase, { fromDate, toDate } = {}) {
  let query = supabase
    ...
    .select(`
      *,
      maintenance_enquiries:enquiry_id (enquiry_number),
      maintenance_clients:client_id (client_name)
    `)
...
export async function fetchFollowUpReportRows(supabase, { fromDate, toDate } = {}) {
  let query = supabase
    ...
    .select(`
      *,
      maintenance_enquiries:enquiry_id (enquiry_number, maintenance_clients:client_id (client_name)),
      maintenance_quotations:quotation_id (quotation_number, maintenance_clients:client_id (client_name))
    `)
...
export async function fetchContractReportRows(supabase, { fromDate, toDate } = {}) {
  let query = supabase
    ...
    .select(`
      *,
      maintenance_quotations:quotation_id (quotation_number),
      maintenance_enquiries:enquiry_id (enquiry_number),
      maintenance_clients:client_id (client_name)
    `)
...
export function applyMaintenanceCustomFilters(rows, filters = {}) {
  let list = [...(rows || [])];
...
export function exportMaintenanceReportCsv(rows, columnIds, fieldDefs, filename) {
  downloadCsv(buildReportCsv(rows, columnIds, fieldDefs), filename);
}

export function formatMaintenanceReportError(err) {
