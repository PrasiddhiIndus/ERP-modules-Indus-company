...
const FollowUpPlanner = () => {
  ...
  const fetchFollowUps = async () => {
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          maintenance_enquiries:enquiry_id (id, enquiry_number, client_id, maintenance_clients:client_id (client_name, contact_email)),
          maintenance_quotations:quotation_id (id, quotation_number, maintenance_clients:client_id (client_name, contact_email))
        `)
        ...
        if (isRevision && followUp.quotation_id) {
          ...
          if (revisionMatch) {
            ...
            const { data: revision } = await supabase
              .from('maintenance_quotation_revisions')
              .select('created_at')
      ...
      processedFollowUps.forEach(followUp => {
        if (followUp.quotation_id) {
          const qNum = followUp.maintenance_quotations?.quotation_number;
          ...
          const exRev = getRevisionNumber(existing.maintenance_quotations?.quotation_number);
          if (curRev > exRev) {
            quotationMap.set(base, followUp);
            return;
          }
  ...
  const fetchSiteVisits = async () => {
    try {
      ...
      // Safety: prevent loading too many rows at once.
  ...
  const handleRevisionSubmit = async (e) => {
    ...
    try {
      ...
      const quotation = selectedFollowUpForRevision.quotation || selectedFollowUpForRevision.maintenance_quotations;
  ...
  const handleView = async (followUp) => {
    ...
    if (followUp.quotation_id) {
      ...
      const baseNumber = followUp.maintenance_quotations?.quotation_number
        ? followUp.maintenance_quotations.quotation_number.split('/R')[0]
        : null;
      if (baseNumber) {
  ...
  const handleExport = () => {
    const exportData = followUps.map(followUp => ({
      ...
      'Enquiry Number': followUp.maintenance_enquiries?.enquiry_number || '-',
      'Quotation Number': followUp.maintenance_quotations?.quotation_number || '-',
      'Client': followUp.maintenance_enquiries?.maintenance_clients?.client_name || '-',
      'Remarks': followUp.remarks || '-',
              ...
              <table className="w-full min-w-[900px]">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {followUps.filter(followUp => {
                    ...
                    const enquiryNumber = followUp.maintenance_enquiries?.enquiry_number?.toLowerCase() || '';
                    const quotationNumber = followUp.maintenance_quotations?.quotation_number?.toLowerCase() || '';
                    const clientName = (followUp.maintenance_enquiries?.maintenance_clients?.client_name || 
                                      followUp.maintenance_quotations?.maintenance_clients?.client_name || '').toLowerCase();
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-sm">
                        <div className="flex flex-col">
                          <span className="font-medium text-gray-900">
                            {followUp.maintenance_enquiries?.enquiry_number || followUp.maintenance_quotations?.quotation_number || '-'}
                          </span>
                          {followUp.maintenance_enquiries?.maintenance_clients?.client_name || followUp.maintenance_quotations?.maintenance_clients?.client_name ? (
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 sm:px-6 py-3 sm:py-4 text-center text-sm font-medium">
                              <button
                                onClick={() => {
                                  navigate('/app/maintenance/expo-seminar');
                                }}
              ...
              <div className="space-y-4">
                {editingFollowUp && editingFollowUp.quotation_id && (
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingFollowUp.maintenance_quotations?.quotation_number || ''}
                      disabled
            ...
            <div className="p-4 sm:p-6 border-b border-gray-200 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs sm:text-sm text-gray-600 mt-1">
                  Quotation: {selectedFollowUpForRevision.maintenance_quotations?.quotation_number || selectedFollowUpForRevision.quotation?.quotation_number || 'N/A'}
                  {selectedFollowUpForRevision.nextRevisionNumber && ` → ${(selectedFollowUpForRevision.maintenance_quotations?.quotation_number || selectedFollowUpForRevision.quotation?.quotation_number || '').split('/R')[0]}/R${selectedFollowUpForRevision.nextRevisionNumber}`}
                </p>
              </div>
              <button
                onClick={() => {
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
            <div className="p-4 sm:p-6 border-b border-gray-200 flex justify-between items-center">
              <div>
                ...
                {viewingFollowUp.maintenance_quotations?.quotation_number && (
                  <p className="text-xs sm:text-sm text-gray-600 mt-1">
                    Quotation: {viewingFollowUp.maintenance_quotations.quotation_number}
                  </p>
                )}
                {viewingFollowUp.maintenance_enquiries?.enquiry_number && (
                  <p className="text-xs sm:text-sm text-gray-600 mt-1">
                    Enquiry: {viewingFollowUp.maintenance_enquiries.enquiry_number}
                  </p>
                )}
              </div>
              <button
