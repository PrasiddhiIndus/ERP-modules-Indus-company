  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (uploadedFiles.length > 0 && enquiryId) {
        ...
        if (uploadFailed > 0) {
          toast.error("Save failed", `Could not save ${uploadFailed} file(s). Error: ${uploadFailed === uploadedFiles.length ? 'Check that table "' + ENQUIRY_DOCUMENTS_TABLE + '" exists (run marketing_enquiry_documents_schema.sql) and storage bucket "' + DOCUMENTS_BUCKET + '" exists.' : 'See console.'}`);
        }
      }
