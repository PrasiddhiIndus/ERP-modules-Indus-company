...
app.post('/api/billing/e-invoice/generate', einvoiceRateLimit, requireBillingAccess, async (req, res) => {
  try {
    ...
    const sellerGstin = String(finalPayload?.SellerDtls?.Gstin || c.gstin || '')
      .trim()
      .toUpperCase();
    const buyerGstin = String(finalPayload?.BuyerDtls?.Gstin || '')
      .trim()
      .toUpperCase();
    const buyerLooksGstin = /^[0-9A-Z]{15}$/.test(buyerGstin);

    // NIC IRN SupTyp must be B2B (or SEZ/export). B2C is rejected as "Invalid value in Type of Supply".
    if (!buyerLooksGstin) {
      return res.status(422).json({
        message:
          'Buyer GSTIN is missing or invalid. E-invoice requires a registered customer GSTIN (B2B).',
      });
    }
    if (sellerGstin && buyerGstin === sellerGstin) {
      return res.status(422).json({
        message:
          'Buyer GSTIN is the same as Indus (seller) GSTIN. ' +
          'Update the buyer on this invoice to the actual customer GSTIN, then generate e-invoice again.',
      });
    }

    // Force valid transaction details for Whitebooks/NIC IRN generate
    ...
    // Never forward internal fields to the provider
    delete finalPayload._meta;
