...
const ExpoSeminar = () => {
  ...
  const fetchExpos = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('maintenance_expo_seminars')
        .select('*')
  ...
  const fetchVisitors = async () => {
    try {
      const { data, error } = await supabase
        .from('maintenance_expo_visitors')
        .select(`
          *,
          maintenance_expo_seminars:expo_id (event_name, event_type)
        `)
      ...
      if (error) {
        ...
        if (error.code === 'PGRST205' || error.message.includes('not found')) {
          console.log('maintenance_expo_visitors table not found, using empty array');
          setVisitors([]);
          return;
        }
        throw error;
        ...
        try {
          ...
          if (uploadError) {
            ...
            if (uploadError.message.includes('Bucket not found') || uploadError.message.includes('not found')) {
              throw new Error('Storage bucket "maintenance-documents" not found. Please create it in Supabase Dashboard > Storage.');
            }
            throw uploadError;
          }
  ...
  const handleSiteVisitSubmit = async (e) => {
    ...
    try {
      ...
      if (error.message.includes('relation') || error.message.includes('does not exist')) {
        toast.warning('Site Visits table not found. Please create the maintenance_site_visits table in your database.');
      } else {
  ...
  const handleExport = () => {
    if (activeTab === 'expos') {
      ...
      const exportData = visitors.map(visitor => ({
        ...
        'Event': visitor.maintenance_expo_seminars?.event_name || '-',
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                  ...
                  <FormDateInput value={formData.start_date} onChange={(e) => setFormData({ ...formData, start_date: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <FormDateInput value={formData.end_date} onChange={(e) => setFormData({ ...formData, end_date: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                    required
                  />
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <FormDateInput value={siteVisitFormData.visit_date} onChange={(e) => setSiteVisitFormData({ ...siteVisitFormData, visit_date: e.target.value })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                      required
                    />
                  </div>
